...
for(var e of array){
    ...
    if(choice === 1){
        if(usedBrowserToTest[0] === "Chrome"){
            ...
            allDev = ['Galaxy S9+_658_320', 'Galaxy S9+_320_658', 'iPhone 13_342_750', 'iPhone 13_664_390', 'Desktop Firefox_720_1280']
            devUA = ['Galaxy S9+_658_320', 'Galaxy S9+_320_658', 'iPhone 13_342_750', 'iPhone 13_664_390',]
        }else{
            devArr = ['iPhone 13', 'Galaxy Note 3']
            allDev = ['iPhone 13_342_750', 'iPhone 13_664_390', 'Galaxy Note 3_640_360', 'Galaxy Note 3_360_640', 'Desktop Safari_720_1280']
            devUA = ['iPhone 13_342_750', 'iPhone 13_664_390', 'Galaxy Note 3_640_360', 'Galaxy Note 3_360_640']
        }
    }else{
        if(choice === 2){
            allDev = ['MobileMBA_640_360', 'MobileMBI_640_360', 'DesktopDBW_720_1280', 'DesktopDBM_720_1280']
        }else{
            allDev = ['MobileMalFormedMBA_720_1280', 'DesktopMAlFormedDBW_720_1280']
        }
    }
    ...
    for(var bro of usedBrowserToTest ){
        ...
        if(choiceRun === 1){
            ...
            console.log(desktop)
            console.log(mobile)
            if(choice === 2 && bro != "WebKit"){
                desktop = ["DesktopDBW_720_1280"]
                mobile = ["MobileMBA_640_360"]
            }
            //web.psuhe(2)
...
function compareHomeWithSubpagesForEachDevices(pageName, comparePath, resultPath, device){
    ...
    jsonsFiles.forEach(filename => {
        ...
        if((filename.includes(`${pageName}.json`) || filename.includes(`${pageName}§`)) && (filename.includes(device.split("_")[0])) && (filename.includes(device.split("_")[1])) && (filename.includes(device.split("_")[2]))) {
            ...
            for (var elem of json_after_split) {
                ...
                if((elem.includes("\":") && !elem.includes("http") && !elem.includes("wss") && !elem.includes("asus-support:")) || elem.includes("visited_Webpage") || elem.includes("visited link") || (elem.includes("total devices:")) || (elem.includes("origins"))){
                    arr_ = elem.split(/":/);
                    //console.log(arr_)
                    header = arr_[0]
                    header_value = arr_[1]
    ...
    if(homePage_CSPHeaderisThere === 'true' && (subPages_cspHeader_Exist === numberOfSubPages)){
        ...
        if(subPages_cspHeader_arr_CSPNoCSPHeader.length > 0){
    ...
    //subPages_Security_arrSafeUnSafeCSP.pushe(2)
...
function compareWebsite(pageName, comparePath, resultPath, devices){
    ...
    jsonsFiles.forEach(filename => {
        let csp = new Array()
        if(filename.includes(`_${pageName}.json`)) {
            const fileData = fs.readFileSync(path.join(comparePath, filename));                    
            const json_arr = JSON.parse(fileData.toString());
            let json_str = JSON.stringify(json_arr);
            json_after_split = json_str.substring(1, json_str.length - 1);
            ...
            if(devices.includes(dev)){
                ...
                for (var elem of json_after_split) {
                    ...
                    if((elem.includes(":") && !elem.includes("http") && !(/.+:\/\//.test(elem)) && !(/.+:$/.test(elem)) && !(/.+:"$/.test(elem)) && !(/.+:"]$/.test(elem)) && !elem.includes("data") && !elem.includes("asus-support:") && !elem.includes("wss") && !elem.includes("mediastream")&& !elem.includes("blob")&& !elem.includes("filesystem")&& !elem.includes("resource")&& !elem.includes(":*")) || elem.includes("visited_Webpage") || elem.includes("dataURL") || elem.includes("origins")){
                        arr_ = elem.split(/":/);
                        header = delete_StrSy(arr_[0], "\"");
...
function compareViewport(pageName, comparePath, resultPath){
    ...
    for(var d of devArr){
        ...
        jsonsFiles.forEach(filename => {
            let csp = new Array()
            if(filename.includes(`_${pageName}.json`) && ((filename.includes(`${dev1Details[0]}_`) &&  filename.includes(dev1Details[1]) && filename.includes(dev1Details[2])) || (filename.includes(`${dev2Details[0]}_`) &&  filename.includes(dev2Details[1]) && filename.includes(dev2Details[2])))) {
                ...
                if(!check){
                    ...
                    for (var elem of json_after_split) {
                        ...
                        //console.log(elem)
                        if((elem.includes(":") && !elem.includes("http") && !(/.+:\/\//.test(elem)) && !(/.+:"$/.test(elem)) && !elem.includes("data") && !elem.includes("asus-support:") && !elem.includes("wss") && !elem.includes("mediastream")&& !elem.includes("blob")&& !elem.includes("filesystem")&& !elem.includes("resource")&& !elem.includes(":*")) || elem.includes("visited_Webpage") || (elem.includes("origins"))){
                            arr_ = elem.split(/":/);
                            //console.log(arr_)
                            header = delete_StrSy(arr_[0], "\"");
        ...
        if(csp_policies.length === 2){
            ...
            safeCSPdev1 = csp_policies[0][1][7][1];
            //console.log(safeCSPdev1)
            //console.log(csp_policies)

            safeCSPdev2 = csp_policies[1][1][7][1];
    ...
    console.log(pageName)
    console.log(devArr.length)
    console.log(devArr)
    console.log(deviceWithDifferentViewPortSafeCSP)
    console.log(deviceWithDifferentViewPortNotSafeCSP)
    console.log(deviceWithDifferentViewPortSafeAndNotSafeCSP)
    if(deviceWithDifferentViewPortSafeCSP === 0 && deviceWithDifferentViewPortNotSafeCSP === 0){
        console.log(safeCSPdev1)
        console.log(safeCSPdev1)
        console.log(deviceWithDifferentViewPortSafeAndNotSafeCSP)
        pagesDViewPSafeAndNotSafeCSPWithDevicesInfoLocalArray.pushe(2)
    }*/
    ...
    jsonsFiles.forEach(filename => {
        ...
        if(filename.includes(`${pageName}.json`) || filename.includes(`${pageName}§`)) {
            ...
            for (var elem of json_after_split) {
                ...
                if((elem.includes("\":") && !elem.includes("http") && !elem.includes("wss") && !elem.includes("asus-support:")) || elem.includes("visited link") || (elem.includes("total devices:"))){
                    arr_ = elem.split(/":/);
                    //console.log(arr_)
                    header = arr_[0]
                    header_value = arr_[1]
